...
# Fetch true final limits
            ...
            if i == 0:
                if i == 0 and j == 0:
                    ax.set_title(f"Network Depth: {col_val}", fontsize=LARGE_FONT, fontweight='bold')
                else:
                    ax.set_title(f"Network Depth: {col_val}", fontsize=LARGE_FONT, fontweight='bold')
    ...
    output_path = os.path.join(os.getcwd(), f"MILP_plot_Network_Depth.pdf")
