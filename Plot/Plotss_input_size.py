...
def plot_scatterplots(merged, column_factor='Input Size'):
    ...
    for i, rate in enumerate(unique_rows):
        for j in range(len(unique_col_values)):
            ...
            if col_val == 'total_finetune':
                ...
                subset = merged[(merged[row_values] == rate) & (merged[column_factor] == 'finetune') & (merged['MagnitudePrune'] == 'Magnitude Pruning') & (merged['PruningType'] == 'unstructured')]
            else:
                subset = merged[(merged[row_values] == rate) & (merged[column_factor] == col_val) & (merged['Finetune'] == 'nonfinetune') & (merged['MagnitudePrune'] == 'Magnitude Pruning') & (merged['PruningType'] == 'unstructured')]
            ...
            if i == 0:
                if col_val == 'nonfinetune':
                    ...
                    if i == 0 and j == 0:
                        ...
                        ax.set_title(f"Input Size: {col_val}", fontsize=LARGE_FONT, fontweight='bold')
                    else:
                        ax.set_title(f"Input Size: {col_val}", fontsize=LARGE_FONT, fontweight='bold')
    ...
    output_path = os.path.join(os.getcwd(), f"fashion_nonfinetune_{column_factor}_new.pdf")
