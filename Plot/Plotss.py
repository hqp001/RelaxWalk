...
def plot_scatterplots(merged, column_factor='Finetune'):
    ...
    fig, axes = plt.subplots(
            ...
            figsize=(5 * len(unique_col_values),
                5 * len(unique_rows)),
    ...
    for i, rate in enumerate(unique_rows):
        for j in range(len(unique_col_values)):
            ...
            if mask_circle.any():
                ax.scatter(dense_time[mask_circle], pruned_time[mask_circle], c=pruned_acc[mask_circle], cmap=custom_cmap, norm=norm, marker='o', edgecolor='k', alpha=0.8)
            if mask_square.any():
                ax.scatter(dense_time[mask_square], pruned_time[mask_square], c=pruned_acc[mask_square], cmap=custom_cmap, norm=norm, marker='s', edgecolor='k', alpha=0.8)
            ...
            ax.tick_params(axis='both', which='major', labelsize=SMALLER_FONT)
    ...
    cbar = fig.colorbar(
        ...
        fraction=0.02,
        pad=0.03  # Adjusted to bring color bar closer
    )
    cbar.set_label("Pruned Accuracy")


    output_path = os.path.join(os.getcwd(), f"mnist_scatterplot_table_{column_factor}.pdf")
